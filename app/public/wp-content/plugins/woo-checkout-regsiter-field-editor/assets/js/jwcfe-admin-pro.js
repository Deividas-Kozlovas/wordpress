var jwcfe_settings = (function($, window, document) {
	...
	var OPTION_ROW_HTML  = '<div class="jwcfe-opt-row">';
        OPTION_ROW_HTML += '<div style="width: 30%; float: left;">';
	    OPTION_ROW_HTML += '<input type="text" name="i_options_key[]" placeholder="Option Value" style="width: 100%;">';
	    OPTION_ROW_HTML += '</div>';
		OPTION_ROW_HTML += '<div style="width: 30%; float: left;">';
	    OPTION_ROW_HTML += '<input type="text" name="i_options_text[]" placeholder="Option Text" style="width: 100%;">';
	    OPTION_ROW_HTML += '</div>';
	    OPTION_ROW_HTML += '<div class="action-cell" style="float: left; margin-right: 5px;">';
	    OPTION_ROW_HTML += '<a href="javascript:void(0)" onclick="jwcfeAddNewOptionRow(this)" class="btn btn-blue" title="Add new option">+</a>';
	    OPTION_ROW_HTML += '</div>';
	    OPTION_ROW_HTML += '<div class="action-cell" style="float: left;">';
	    OPTION_ROW_HTML += '<a href="javascript:void(0)" onclick="jwcfeRemoveOptionRow(this)" class="btn btn-red" title="Remove option">x</a>';
	    OPTION_ROW_HTML += '</div>';
	    OPTION_ROW_HTML += '<div class="action-cell sort ui-sortable-handle"></div>';
		OPTION_ROW_HTML += '</div>';
...
if (name == '') {
    ...
    nameInput.focus();
    document.getElementById('nameInput').scrollTo({ top: 0, behavior: 'smooth' });
    document.getElementById('nameInput').style.borderColor = 'red';
    document.getElementById('nameInput').style.boxShadow = 'unset';
} else if (!isHtmlIdValid(name)) {
    err_msgs = MSG_INVALID_NAME;
    document.getElementById('nameInput').focus();
    nameInput.focus();
...
_enableDisableSelectedFields = function enableDisableSelectedFields(enabled) {
	...
	*------------------------------------*/
			...
			try{
				...
				if(optionsList){
					jQuery.each(optionsList, function() {
						...
						var html  = '<div class="jwcfe-opt-row" style="margin-top:10px;">';
						html += '<div style="width: 30%; float: left;margin-top: 5px;"><input type="text" name="i_options_key[]" value="'+newkey+'" placeholder="Option Value" style="width: 100%;background-color: #f1f2f6; border-color:lightgray; "></div>';
						html += '<div style="width: 30%; float: left;margin-top: 5px;"><input type="text" name="i_options_text[]" value="'+newtxt+'" placeholder="Option Text" style="width: 100%;background-color: #f1f2f6; border-color:lightgray; "></div>';
						
						
						html +='<div class="action-cell" style="float: left; margin-right: 5px;"><a href="javascript:void(0)" onclick="jwcfeAddNewOptionRow(this)" class="btn btn-blue" title="Add new option">+</a></div>';
						html += '<div class="action-cell" style="float: left; margin-right: 5px;"><a href="javascript:void(0)" onclick="jwcfeRemoveOptionRow(this)" class="btn btn-red"  title="Remove option">x</a></div>';
						html += '<div class="action-cell sort ui-sortable-handle"></div>';
						html += '</div>';
	...
	addNewOptionRow = function addNewOptionRow(elm){
		...
		var ptable = $(elm).closest('.jwcfe-opt-container');
