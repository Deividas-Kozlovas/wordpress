<?php
/**
 * Plugin Name: WooCommerce Checkout Field Editor (Checkout Manager)
...
add_action( 'admin_enqueue_scripts', 'jwcfe_admin_scripts', 10, 1 );
	...
	function jwcfe_billing_fields_lite($fields, $country){
		...
		if(is_wc_endpoint_url('edit-address')){
			return $fields;
				...
				return jwcfe_prepare_address_fields(get_option('jwcfe_wc_fields_billing'), $country, $fields, 'billing');
	...
	function jwcfe_shipping_fields_lite($fields, $country){
		...
		if(is_wc_endpoint_url('edit-address')){
			...
			return jwcfe_prepare_address_fields(get_option('jwcfe_wc_fields_shipping'), $country, $fields, 'shipping');
		}
	}
	 ...
	 *
	 */
	function jwcfe_prepare_address_fields($fieldset, $country, $original_fieldset = false, $sname = 'billing') {
...
// add_action( 'upgrader_process_complete', 'jwcfe_upgrade_function',10, 2);
// function jwcfe_upgrade_function( $upgrader_object, $options ) {
...
//        	//verify the pluginn name
//           if ($each_plugin==$current_plugin_path_name) {



             
//           	if (get_option('wc_fields_billing', false)) {
//           		if(!empty(get_option('wc_fields_billing', false))){
//           			$old_billing_fields = get_option('wc_fields_billing', false);
//           			update_option('jwcfe_wc_fields_billing',$old_billing_fields);
...
// 					if (get_option('wc_fields_shipping', false)) {
//           		if(!empty(get_option('wc_fields_shipping', false))){
//           			$old_shipping_fields = get_option('wc_fields_shipping', false);
//           			update_option('jwcfe_wc_fields_shipping',$old_shipping_fields);
...
// 						if (get_option('wc_fields_additional', false)) {
//           		if(!empty(get_option('wc_fields_additional', false))){
//           			$old_additional_fields = get_option('wc_fields_additional', false);
//           			update_option('jwcfe_wc_fields_additional',$old_additional_fields);
          			
//           		}
// 					}

// 						if (get_option('wc_fields_account', false)) {
//           		if(!empty(get_option('wc_fields_account', false))){
//           			$old_account_fields = get_option('wc_fields_account', false);
//           			update_option('jwcfe_wc_fields_account',$old_account_fields);
//           		     }
//                 }
